...
export function useNotifications() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        Log("frontend", "warn", "api", `API returned status ${res.status}`);
      } catch {
        Log("frontend", "error", "api", "Network error fetching notifications");
      }
  ...
  const markAsViewed = useCallback((id: string) => {
    setViewedIds((prev) => {
      ...
      Log("frontend", "debug", "hook", `Notification ${id} marked as viewed`);
      return next;
    });
  }, []);

  return { notifications, viewedIds, loading, markAsViewed };
}
